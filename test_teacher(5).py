...
def _v_and_grad(model, x, value_decode, q_mean, q_std):
    """x: (B, obs_dim) normalized. Returns V, ||dV/dx||, dV/dx."""
...
def value_grad_stats(
    ...
    """g(s) = ||∇_x V(x)||, x = norm_obs(s), V = decode(v_θ(x)).

    Name: value-gradient norm (critic Lipschitz diagnostic).
    Also mean cosine(∇V, x_g - x) on near states.
    """
    device = check_device()
    ns = 0 if critic_checkpoint == -1 else critic_checkpoint
    stats = get_critic_stats(dataset_name, specific_dataset, task_id, ns)
