...
def plot_reward_heatmap(STEP, agg_method='max', highlight_negatives=True):
    """
    Plot critic heatmap using only 2D state vector [x, y].
    This function extracts only position information and uses mean velocities from training.
    """
    ...
    print(obs_dim)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = SimpleReward(obs_dim, act_dim).to(device)
    model.load_state_dict(state_dict)
    model.eval()
    stats = get_pretrained_reward_stats(name)

    print(f"Reward Model loaded. Observation dimension: {obs_dim}")
    print(f"Observation mean: {stats.obs_mean}")
    print(f"Observation std: {stats.obs_std}")
    ...
    for goal_idx, goal in enumerate(GOALS):
        ...
        for start in range(0, len(obs_base), BATCH_SIZE):
            ...
            # Differentiable normalization
            obs_norm = (batch_obs - obs_mean_t) / obs_std_t
            act = torch.zeros((batch_obs.shape[0], act_dim), dtype=torch.float32)
            r = model(obs_norm.to(device), act.to(device))  # [B]
            reward_map_goal[start:end] = r.detach().cpu().numpy()
