...
class OnlineFinetuner():
    def __init__(self, config: FinetuningConfig):
        ...
        if self.config.finetune_batch_size % self.accelerator.num_processes != 0:
            raise ValueError(
                "finetune_batch_size must be divisible by the number of processes "
                f"when split_batches=True: {self.config.finetune_batch_size} vs "
    ...
    def finetune_planner(self):
        ...
        for step in range(self.config.finetune_rounds):
            ...
            dataloader = DataLoader(
                ...
                # PlannerDataset is already an in-memory tensor list and each
                # offline round has only a few steps; worker startup/teardown
                # costs more than direct loading and previously multiplied CPU
                # usage by every rank.
                num_workers=0,
                generator=generator,
            )
