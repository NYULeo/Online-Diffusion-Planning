...
"""
def save_model(kernel_net, name, num_steps, ensemble_idx):
    kernel_net.eval()
    net_dict =  kernel_net.state_dict()
    os.makedirs(f'./Finetuning/Kernels/{name}/{num_steps}', exist_ok=True)
    save_path = f'./Finetuning/Kernels/{name}/{num_steps}/{num_steps}_{ensemble_idx}.pkl'
    torch.save(net_dict, save_path)
    print(f"Kernel model save to {name}_{num_steps}_{ensemble_idx}.pkl")
...
def load_model(name, steps, ensemble_idx):
    load_path = f'./Finetuning/Kernels/{name}/{steps}/{name}_{steps}_{ensemble_idx}.pkl'
...
class KernelDataset(Dataset):
    ...
    def save_stats(self, name, step):
        stats_name =  str(name) + '_'+ str(step)+ '_stats.pkl'
        stats_dir = f'./Finetuning/Kernels/{name}/{step}/Stats/'
...
def get_pretrained_kernel(dataset_name, step, specific_dataset: Optional[str] = None):
       _, name, obs_dim, act_dim  =  Train_Dataset(dataset_name, specific_dataset)
       path = f'./Finetuning/Kernels/{name}/{step}'
       file_count = count_files_in_folder(path)
       kernel_state_dicts = []
       for i in range(file_count):
           kernel_state_dicts.append(load_model(name, step, i))
       return kernel_state_dicts, obs_dim, act_dim, name



def get_pretrained_kernel_stats(name, step):
     stats_path = f'./Finetuning/Kernels/{name}/{step}/Stats/{name}_{step}_stats.pkl'
     with open(stats_path, 'rb') as f:
        stats = pickle.load(f)
     return stats
