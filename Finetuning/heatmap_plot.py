...
def reward_heatmap(STEP, env_name, specific_env, hidden_layers, hidden_dim, agg_method='max', highlight_negatives=True):
    ...
    # Use full maze bounds (prevents cropping for pointmaze/large)
    if not (hasattr(env, "maze") and hasattr(env.maze, "map_length") and hasattr(env.maze, "map_width")):
         raise ValueError("env.maze is missing map dimensions; can't set full bounds.")

    H = env.maze.map_length
    W = env.maze.map_width
    cell = env.maze.maze_size_scaling
 
    grid_min = (np.array([-W / 2 * cell, -H / 2 * cell], dtype=np.float32) - GRID_MARGIN)
    grid_max = (np.array([ W / 2 * cell,  H / 2 * cell], dtype=np.float32) + GRID_MARGIN)
