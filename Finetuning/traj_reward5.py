...
class TotalReward_Critic(nn.Module):
    ...
    def predict(self, x: torch.Tensor, lam: float):
        ...
        for i in range(H-1):
            ...
            r = self.reward_net(s_norm_reward, a)
            total_reward += ((H-(i+1)) * ((self.config.critic_gamma**i)*(r.squeeze(0))))
        ...
        for i in range(1, H):
            ...
            total_reward +=   ((self.config.critic_gamma**(i)) * (  (self.q_stats.Q_std * v.squeeze(0)) + self.q_stats.Q_mean  )  )
    ...
    def forward(self, x: torch.Tensor, lam: float):
        ...
        for i in range(H-1):
            ...
            r_a = grads[1].squeeze(0)
            r_s_grad, r_a_grad = self.makeGrad(H, r_s, r_a, i)
            gradient +=  ( (H-(i+1)) * ((self.config.critic_gamma**i)*(r_s_grad + r_a_grad)) )
            total_reward += ( (H-(i+1)) * ((self.config.critic_gamma**i)*(r.squeeze(0))) )
        ...
        for i in range(1, H):
            ...
            grad_critic = self.makeGrad_Critic(H, v_s, i)
            gradient +=   ( (self.config.critic_gamma**(i)) *  (self.q_stats.Q_std * grad_critic))
            total_reward +=   ((self.config.critic_gamma**(i)) * (  (self.q_stats.Q_std * v.squeeze(0)) + self.q_stats.Q_mean  )  )
        
        gradient = gradient * (1/(H-1))
        total_reward = total_reward * (1/(H-1))
...
class TotalReward_Critic(nn.Module):
    ...
    def forward(self, x: torch.Tensor, lam: float):
        ...
        for i in range(1, H):
            ...
            v_s = grads[0].squeeze(0) * torch.tensor((1/np.maximum(self.critic_stat.obs_std, self.critic_stat.std_floor)), device = self.config.device, dtype=torch.float32, requires_grad = False)
            grad_critic = self.makeGrad_Critic(H, v_s, i)
            gradient +=   ( (self.config.critic_gamma**(i)) *  ( grad_critic))
            total_reward +=   ((self.config.critic_gamma**(i)) * (  ( v.squeeze(0))  )  )
